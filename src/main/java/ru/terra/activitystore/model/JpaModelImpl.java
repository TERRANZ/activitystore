...
import ru.terra.activitystore.db.entity.*;
import ru.terra.activitystore.db.manager.*;
...
public class JpaModelImpl extends ActivityStoreModel {
    ...
    @Override
    public void start() {
        if (bc.getBlockCount() == 0) {
            //first start
            Block firstBlock = new Block();
            firstBlock.setId(0);
            firstBlock.setCreationDate(new Date());
            firstBlock.setName("root");
            firstBlock.setParent(0);
            firstBlock.setUpdateDate(new Date());
            bc.create(firstBlock);
        }
    }
